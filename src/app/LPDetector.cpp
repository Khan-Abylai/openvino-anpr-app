...
Detector::Detector(float threshold):threshold(threshold) {
    try {
        ov::Core core;
...
std::vector<std::shared_ptr<LicensePlate>> Detector::detect(const cv::Mat& image) {
    ...
    auto filtered_predictions = nms(predictions);
    ...
    if (filtered_predictions.empty()) {
        return vector<shared_ptr<LicensePlate>>{};
    }else{
        for (auto& pred : filtered_predictions) {
            ...
            auto licensePlate = make_shared<LicensePlate>(
                x_c,
                y_c,
                width,
                height,
                static_cast<int>(src_points[0].x),
                static_cast<int>(src_points[0].y),
                static_cast<int>(src_points[2].x),
                static_cast<int>(src_points[2].y),
                static_cast<int>(src_points[1].x),
                static_cast<int>(src_points[1].y),
                static_cast<int>(src_points[3].x),
                static_cast<int>(src_points[3].y),
                conf
                );
            filteredLicensePlates.emplace_back(licensePlate);
        }
